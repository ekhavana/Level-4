"""
Terrace Gallery 1 Touch Panel — Roku source, local Frame TV volume, TV1 power.
...
dvTLP = devices.dvTerraceGalleryTLP1
...
BtnSourceRoku = Button(dvTLP, 1001)
BtnTVPowerOn = Button(dvTLP, 257)
BtnTVPowerOff = Button(dvTLP, 258)
BtnMute = Button(dvTLP, 1003)
VolumeSlider = Slider(dvTLP, 1002)

TVPowerBtns = MESet([BtnTVPowerOff, BtnTVPowerOn])
SourceBtns = MESet([BtnSourceRoku])

BtnRokuRev = Button(dvTLP, 1004)
BtnRokuPlay = Button(dvTLP, 1005)
BtnRokuFwd = Button(dvTLP, 1006)
BtnRokuLeft = Button(dvTLP, 1008)
BtnRokuUp = Button(dvTLP, 1009)
BtnRokuRight = Button(dvTLP, 1010)
BtnRokuDown = Button(dvTLP, 1011)
BtnRokuSelect = Button(dvTLP, 1012)
# Former Netflix button (join 1013) — layout relabeled to Hulu.
BtnRokuHulu = Button(dvTLP, 1013)
BtnRokuHome = Button(dvTLP, 1014)
BtnRokuBack = Button(dvTLP, 1016)
BtnRokuYoutube = Button(dvTLP, 1017)

av.RegisterTVLocalVolumeSlider('TerraceTV1', VolumeSlider)


def _show_roku_popup():
    SourceBtns.SetCurrent(BtnSourceRoku)
    TVPowerBtns.SetCurrent(BtnTVPowerOn)
    dvTLP.ShowPopup(variables.POPUPS['Roku TV 1'])
    av.RefreshTVLocalVolumeUI('TerraceTV1')
